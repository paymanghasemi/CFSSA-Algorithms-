...
void Writing_Output() {
    ...
    OutputFile << IP_file_name << " GLB= " << std::setprecision(Output_Precision) << Global_LB << " GUB= " << Global_UB << " Gap= " << (Global_UB - Global_LB) / (Global_UB + denominator) << " #VAR= " << N_Variables - N_Objectives << " #Const= " << N_Constraints << " Time= " << (clock_Run_time / CLOCKS_PER_SEC) << " #IP= " << N_LP << " tune= " << tune_Run_time << endl;
    OutputFile.close();
}
